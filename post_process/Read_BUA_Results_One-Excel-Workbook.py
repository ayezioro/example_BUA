...
def main():
    ...
    # output_folder = r"C:\Users\YourName\Documents\EnergyResults"  # <<< UPDATE this path
    output_folder = path_dir_simulation_all_alternatives
    os.makedirs(output_folder, exist_ok=True)

    # === Load JSON ===
    # json_path = r"..\..\..\..\OneDrive - Technion\GH\Tanya\14\TestAllRuns_Pycharm\results.json"  # <<< UPDATE if needed
    ...
    # === 3. Extract KPIs for each scenario ===
    kpi_records = []
    for case_name, case_data in data.items():
        kpis_all = case_data.get("kpis", {})

        for scenario_name, kpis in kpis_all.items():
            record = {"case": case_name, "scenario": scenario_name}
    ...
    combined_excel_path = os.path.join(output_folder, "energy_results_combined.xlsx")
    with pd.ExcelWriter(combined_excel_path, engine='openpyxl') as writer:
        df_bes.to_excel(writer, sheet_name="Building_Energy", index=False)
        df_ubes.to_excel(writer, sheet_name="Urban_Energy", index=False)
        df_kpis.to_excel(writer, sheet_name="KPIs", index=False)

    print(f"✅ Export complete.\nExcel file with 3 sheets saved to:\n{combined_excel_path}")
